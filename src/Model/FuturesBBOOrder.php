...
/**
 * FuturesBBOOrder Class Doc Comment
 *
 * @category Class
 * @description contractBBOorderdetails
 * @package  GateApi
 * @author   Gate
 * @link     https://www.gate.com
 */
class FuturesBBOOrder implements ModelInterface, ArrayAccess
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'FuturesBBOOrder';
     ...
     * Sets direction
     *
     * @param string $direction Direction: 'sell' fetches the bid side, 'buy' fetches the ask side.
     *
     * @return $this
     */
     ...
     * Sets level
     *
     * @param int $level Level: maximum 20 levels
     *
     * @return $this
     */
     ...
     * Sets text
     *
     * @param string|null $text 订单自定义信息，用户可以用该字段设置自定义 ID，用户自定义字段必须满足以下条件：  1. 必须以 `t-` 开头 2. 不计算 `t-` ，长度不能超过 28 字节 3. 输入内容只能包含数字、字母、下划线(_)、中划线(-) 或者点(.)  除用户自定义信息以外，以下为内部保留字段，标识订单来源:  - web: 网页 - api: API 调用 - app: 移动端 - auto_deleveraging: 自动减仓 - liquidation: ⽼经典模式仓位强制平仓 - liq-xxx: a. 新经典模式仓位强制平仓，包含逐仓、单向全仓、双向全仓⾮对冲仓位强平。 b. 统⼀账户单币种保证金模式逐仓强制平仓 - hedge-liq-xxx: 新经典模式双向全仓对冲部分强制平仓，即同时平多空仓位 - pm_liquidate: 统⼀账户跨币种保证金模式强制平仓 - comb_margin_liquidate: 统⼀账户组合保证金模式强制平仓 - scm_liquidate: 统⼀账户单币种保证金模式仓位强制平仓 - insurance: 保险
     *
     * @return $this
     */
